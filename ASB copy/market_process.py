...
class StockPriceProcess:
    """
    Geometric Brownian Motion for stock price evolution.
    
    S_{n+1} = S_n * exp((μ - σ²/2)δt + σ√δt ε_{n+1}), where ε_n are i.i.d. N(0,1) random variables
    """
    ...
    def __init__(self, 
        ...
        Args:
            ...
            mu: Drift parameter (μ in the formula) - expected annual return rate
            sigma: Volatility parameter (σ in the formula) - annual volatility
            dt: Time step size (δt in the formula) - 1/252 for daily steps (252 trading days per year)
        """
        self.S0 = S0
        self.mu = mu  # Drift parameter for geometric BM
        self.sigma = sigma
        self.dt = dt
    ...
    def simulate_path(self, 
        ...
        """
        Simulate stock price paths using Geometric Brownian Motion with tensors.
        
        S_{n+1} = S_n * exp((μ - σ²/2)δt + σ√δt ε_{n+1}), where ε_n are i.i.d. N(0,1)
        ...
        drift_term = (self.mu - 0.5 * self.sigma**2) * dt_tensor
        diffusion_term = self.sigma * torch.sqrt(dt_tensor)
        
        for i in range(N):
            log_return = drift_term + diffusion_term * epsilon[:, i]
            S[:, i + 1] = S[:, i] * torch.exp(log_return)
...
def update_cash_spent(X_prev: Union[torch.Tensor, float, np.ndarray], 
    ...
    if isinstance(X_prev, torch.Tensor):
        ...
        return X_prev + v * S_next + execution_cost
    else:
...
if __name__ == "__main__":
    ...
    process = StockPriceProcess(S0=45.0, mu=0.0, sigma=0.2, dt=1/252)  # 20% annual volatility, daily time steps
    ...
    print(f"Stock Price Process: Geometric Brownian Motion")
    print(f"Formula: S_{{n+1}} = S_n * exp((μ - σ²/2)δt + σ√δt ε_{{n+1}})")
    print(f"Parameters:")
    print(f"  - Initial price (S₀): €{process.S0:.2f}")
    print(f"  - Drift (μ): {process.mu:.1%}")
    print(f"  - Volatility (σ): {process.sigma:.1%}")
    print(f"  - Time step (δt): {process.dt:.4f} (1/252 of a year = 1 trading day)")
    ...
    test_cost = execution_cost_function(test_rho, eta=0.0, phi=0.5)
